...
public class BGMPlayer implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (audioClip != null) {
                ...
                System.out.println("[디버그] 배경음악 재생 시작: " + audioId);
                ...
                while (isPlaying && !stopRequested) {
                    try {
                        ...
                        System.out.println("[디버그] 배경음악 쓰레드 인터럽트됨: " + e.getMessage());
                        break;
                    }
                }
                
                System.out.println("[디버그] 배경음악 재생 종료: " + audioId);
            ...
            System.err.println("[디버그] 배경음악 재생 중 오류 발생: " + e.getMessage());
            e.printStackTrace();
        } finally {
            cleanup();
        }
    ...
    public synchronized void pause() {
        if (isPlaying && !isPaused && audioClip != null) {
            isPaused = true;
            audioClip.stop();
            System.out.println("[디버그] 배경음악 일시정지: " + audioId);
        }
    }
    ...
    public synchronized void resume() {
        if (isPlaying && isPaused && audioClip != null) {
            ...
            System.out.println("[디버그] 배경음악 재개: " + audioId);
        }
    }
    ...
    public synchronized void requestStop() {
        ...
        System.out.println("[디버그] 배경음악 중지 요청: " + audioId);
    }
    ...
    private void cleanup() {
        ...
        System.out.println("[디버그] 배경음악 리소스 정리 완료: " + audioId);
    }
